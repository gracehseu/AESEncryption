# global variable 
roundConstant = [
  [0x00, 0x00, 0x00, 0x00],
  [0x01, 0x00, 0x00, 0x00],
  [0x02, 0x00, 0x00, 0x00],
  [0x04, 0x00, 0x00, 0x00],
  [0x08, 0x00, 0x00, 0x00],
  [0x10, 0x00, 0x00, 0x00],
  [0x20, 0x00, 0x00, 0x00],
  [0x40, 0x00, 0x00, 0x00],
  [0x80, 0x00, 0x00, 0x00],
  [0x1b, 0x00, 0x00, 0x00],
  [0x36, 0x00, 0x00, 0x00],
]

sBox = []

# AES uses a key schedule to expand a short key into a number of separate round keys
# https://en.wikipedia.org/wiki/Rijndael_key_schedules
def keyExpansion(key_file, key_length, num_rounds):
  key_schedule = []
  # print("key_length: " + str(key_length))

  for i in range(key_length):
    key_row = []
    key_row.append(key_file[4 * i])
    key_row.append(key_file[4 * i + 1])
    key_row.append(key_file[4 * i + 2])
    key_row.append(key_file[4 * i + 3])

    key_schedule.append(key_row)

  # key expansion
  for i in range(key_length, 4 * (num_rounds + 1)):
    temp = []
    key_schedule.append([])

    for t in range(4):
      temp.append(key_schedule[i - 1][t])

    if (i % key_length == 0):
      temp = subWord(rotWord(temp), sBox)
      for t in range(len(temp)):
        temp[t] ^= roundConstant[i // key_length][t]

    elif key_length > 6 and i % key_length == 4:
      temp = subWord(temp)

    for t in range(4):
      key_schedule[i].append(key_schedule[i - key_length][t] ^ temp[t])

  return key_schedule


# applies the S-box transformation to every word
def subWord(word, Sbox):
  for i in range(len(word)):
    word[i] = Sbox[word[i]]
  return word

# rotates every word one byte to the left
def rotWord(word):
  # temp valuable for keeping first word before shift destroys
  first = word[0]
  # shift one byte over
  for i in range(len(word) - 1):
    word[i] = word[i + 1]
  # last word is now first 
  word[len(word) - 1] = first
  return word

# addititon in place, AES addition is XOR
def addRoundKey(state, key_schedule, current_round):
  for row in range(4):
    for column in range(4):
      state[row][column] ^= key_schedule[current_round * 4 + row][column]
  return state